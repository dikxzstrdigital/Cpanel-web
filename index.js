...
const IdBuyerSeikyouStoreDbPath = './db/IdBuyerSeikyouStore.json';
...
if (!fs.existsSync(IdBuyerSeikyouStoreDbPath)) {
  fs.writeFileSync(IdBuyerSeikyouStoreDbPath, JSON.stringify([], null, 2));
  console.log('Membuat file db/IdBuyerSeikyouStore.json. Harap isi dengan ID Telegram buyer SeikyouStore.');
}
...
app.post('/cpanel', async (req, res) => {
  ...
  if (isPanel3User) {
    ...
    buyerType = null; // Tidak perlu validasi ID SeikyouStore
    ...
    // buyerType (SeikyouStore/luar) dihormati
  ...
  try {
    ...
    // --- Validasi ID SeikyouStore (HANYA jika buyerType adalah 'SeikyouStore') ---
    if (buyerType === 'SeikyouStore') {
      logActivity(actionUser, 'Proses sebagai Buyer SeikyouStore. Memeriksa ID Telegram...');
      let IdBuyerSeikyouStore = [];
      try {
        IdBuyerSeikyouStore = JSON.parse(fs.readFileSync(IdBuyerSeikyouStoreDbPath, 'utf8'));
      } catch (dbError) {
        console.error(`Gagal membaca ${IdBuyerSeikyouStoreDbPath}:`, dbError);
        return res.status(500).json({ status: false, message: 'Gagal memvalidasi database buyer.' });
      }

      if (!IdBuyerSeikyouStore.includes(telegramId)) {
        logActivity(actionUser, `Gagal: ID Telegram ${telegramId} tidak ada di db/IdBuyerSeikyouStore.json.`);
        return res.status(403).json({ status: false, message: `ID Telegram ${telegramId} tidak terdaftar sebagai Buyer SeikyouStore. Gagal membuat panel.` });
      }
      logActivity(actionUser, `ID Telegram ${telegramId} tervalidasi.`);
    }
    // --- Akhir Validasi ID SeikyouStore ---
    ...
    const username = nama;
    const email = username + "@SeikyouStore.PANEL";
...
app.post('/api/addserver', async (req, res) => {
  ...
  try {
    ...
    // --- Validasi User (Cek Batas Server & Validasi SeikyouStore) ---
    ...
    // --- VALIDASI SeikyouStore (Hanya relevan jika buyerType adalah 'SeikyouStore') ---
    if (buyerType === 'SeikyouStore') {
      ...
      if (!IdBuyerSeikyouStore.includes(lastNameAsTelegramId)) {
        logActivity(actionUser, `Gagal: User ID ${userId} (Tele ID: ${lastNameAsTelegramId}) tidak ada di db/IdBuyerSeikyouStore.json.`);
        return res.status(403).json({ status: false, message: `Gagal menambah server: User ID ${userId} tidak terdaftar sebagai Buyer SeikyouStore.` });
      }
      logActivity(actionUser, `User ID ${userId} tervalidasi sebagai Buyer SeikyouStore.`);
    }
...
app.listen(port, '0.0.0.0', () => {
  ...
  console.log(`SeikyouStore CPanel berjalan di http://${ip}:${port}`);
});